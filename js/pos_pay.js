import { getCartItems, getSelectedCustomerId, getCurrentSessionId, setSelectedCustomerId } from './pos_state.js';
...
export async function getPaymentData() {
  ...
  const exchangeId = document.getElementById("multiExchangeId")?.value.trim() || null;
  const exchangeText = document.getElementById("multiExchangeInfo")?.textContent || "";
  const exchangeAmount = exchangeText.includes("Rs") ? parseFloat(exchangeText.replace(/\D/g, "")) || 0 : 0;
  ...
  if (cashAmount > 0) {
    payments.push({ method: 'cash', amount: cashAmount, cashIn, change: Math.max(0, cashIn - cashAmount) });
  }
  if (cardAmount > 0) {
  ...
  export async function finalizePayment() {
    ...
    const balanceDue = parseFloat(document.getElementById("multiBalanceDue")?.textContent || 0);
    ...
    try {
      ...
      const orderTotal = parseFloat((subtotal * (1 - billDiscount / 100)).toFixed(2));
      const paymentMethod = payments.length === 1 ? payments[0].method : 'multy';
...
closeModal("customerModal");
      ...
      document.getElementById("payment-error-message").textContent = "❌ Payment failed. Try again.";
    }
  }
