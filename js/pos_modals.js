...
import { getSessionActive, getCurrentSessionId, getExchangeItems, removeExchangeItem, clearExchangeItems } from './pos_state.js';
...
import { resetPOSState } from './pos_pay.js';
  ...
  export function showPriceSelectionModal(productName, stockOptions, callback) {
    ...
    stockOptions.forEach((stock) => {
      ...
      btn.textContent = `Rs ${parseFloat(stock.sell_price).toFixed(2)} (Qty: ${stock.quantity})`;
      btn.onclick = () => {
    ...
    exchangeItems.forEach((item, index) => {
      ...
      row.innerHTML = `
        ...
        <div class="cart-actions">
          <label>Qty:</label>
          <input type="number" value="${item.quantity}" min="1" onchange="window.updateExchangeQty(${index}, this.value)" />
          <div class="item-total">Rs ${subtotal.toLocaleString("en-LK", { minimumFractionDigits: 2 })}</div>
  ...
  export async function renderReceiptAndShopCopy(
    ...
    if ((paymentMethod === "cash" || paymentMethod === "multy") && cashInAmount !== null) {
    ...
    if ((paymentMethod === "cash" || paymentMethod === "multy") && cashInAmount !== null) {
      ...
      balText.innerHTML = `<strong>Balance Given:</strong> Rs ${balance?.toFixed(2) || "0.00"}`;
